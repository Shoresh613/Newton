...
void read_constants(void) /* Denna ropas upp vid boot */
{
	int i=1, end=0;
	FILE *fp=NULL;
	char *Constantsfile_error=NULL, *ConstantTooLong=NULL, *ConstantSyntax=NULL, *home=NULL, *tmp=NULL;
	...
	else  /* om $HOME „r satt */
	{
		...
		if((fp=fopen(tmp,"r"))== NULL)
		{
			strcpy(tmp,home);
			strcat(tmp,"\\");
			strcat( tmp, "newton.cnf" );
			if((fp=fopen(tmp,"r"))== NULL)
			{
				if((fp=fopen("newton.cnf","r"))== NULL) /* L„ser „ven i vanliga directoriet */
				{
					form_alert( 1, Constantsfile_error);
					Mfree(tmp); fclose(fp);
					return;
	...
	do
	{
		...
		if( !((konstant_lista[i][0]<='Z') && (konstant_lista[i][0]>='A')) )	/* Om inte stor bokstav */
		{
